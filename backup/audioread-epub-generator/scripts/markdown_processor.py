...
class MarkdownProcessor:
    ...
    def __init__(self):
        ...
        self.image_map: Dict[str, str] = {}  # 原始路径 -> EPUB 内路径
    ...
    def set_image_map(self, image_map: Dict[str, str]) -> None:
        """
        Set the image path mapping for EPUB generation.

        Args:
            image_map: Dict mapping original paths to EPUB internal paths
        """
    ...
    def markdown_to_html(markdown_text: str) -> str:
        ...
        for line in lines:
            ...
            # Handle standalone images (image on its own line)
            ...
            if img_match:
                ...
                alt_text = MarkdownProcessor.escape_html(alt_text)
                html_parts.append(f'<div class="image-container"><img src="{img_src}" alt="{alt_text}"/></div>')
                continue
    ...
    @staticmethod
    def _render_inline(text: str, image_map: Dict[str, str] = None) -> str:
        """
        Render inline markdown elements (bold, italic, links, code, images).

        Args:
            text: Text with inline markdown
            image_map: Optional dict mapping original image paths to EPUB paths
        ...
        # Handle inline images BEFORE escaping HTML (since ![...] contains special chars)
        def replace_image(match):
            alt = match.group(1)
            src = match.group(2)
            # Map path if mapping exists
            epub_src = image_map.get(src, src)
            # Escape alt text
            safe_alt = (alt
                       .replace('&', '&amp;')
                       .replace('<', '&lt;')
                       .replace('>', '&gt;')
                       .replace('"', '&quot;'))
            return f'<img src="{epub_src}" alt="{safe_alt}" class="inline-image"/>'

        text = re.sub(r'!\[([^\]]*)\]\(([^)]+)\)', replace_image, text)

        # Escape HTML special characters
        # But preserve our already-inserted <img> tags
        # Use placeholder to protect img tags
        img_tags = []
        def save_img(match):
            img_tags.append(match.group(0))
            return f'__IMG_TAG_{len(img_tags)-1}__'

        text = re.sub(r'<img [^>]+/>', save_img, text)
        ...
        text = re.sub(r'__IMG_TAG_(\d+)__', restore_img, text)
